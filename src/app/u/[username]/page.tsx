...
export default function SendMessage() {
  ...
  const fetchSuggestedMessages = async () => {
    ...
    try {
      ...
      const list = parseStringMessages(data.message || '');
      if (!list.length) {
        setSuggestedMessages(parseStringMessages(initialMessageString));
        toast.message('Using default suggestions.');
      } else {
        setSuggestedMessages(list);
      }
    } catch (error) {
      console.error('Error fetching messages:', error);
      setSuggestedMessages(parseStringMessages(initialMessageString));
      toast.error('Failed to load suggested messages. Showing defaults.');
    } finally {
        ...
        <Card>
          ...
          <CardContent className="flex flex-col space-y-4">
            {suggestedMessages.length > 0 ? (
              ...
              <p className="text-gray-500">No suggested messages yet. Click the button above!</p>
            )}
          </CardContent>
        </Card>
      </div>
